...
3. **Do NOT rebuild the site.** Only T6 final reviewer rebuilds the site.
...
2. The T1b task renames ALL files and inline content: (a) rename files in archive, refined, drafts, critiques, names, docs, etc., (b) replace every occurrence of the old name inside each file — H1, identity line, body text — using `grep -r "<old-name>" .` to find them all. Do NOT rebuild the site — only T6 does that. Then create a fresh T3 task (with the T1b as parent) to re-review the renamed soul.
