...
**Kill 2 — no failure mode (v5.3.0, evidence: 2026-08-12 backtest).** A seed whose character cannot plausibly fail is not a character — it's an essay topic. "Promise-collector," "apology-maker," "scale-keeper," "being told you've done enough" — these are states, not people. If nothing can genuinely go wrong in the world of the concept (the ledger never breaks, the scale never tips, the apology always lands), the seed is word salad waiting to happen, no matter how rich the sensory detail. Kill it. The strongest seeds name a material practice with its own failure modes: a lock that won't open, a record that gets erased, a threshold that lets the wrong one in, a log that lies, a splinter that festers. Backtested 2026-08-12: correctly archived Talley (promise-collector) and Mendel (tailor of unsaid things), kept all 11 live souls including borderline cases (Everson, Peebles). False-positive rate: 0%.
