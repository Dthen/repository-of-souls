...
**Read the verdict ledger too (`seeds/VERDICT_LEDGER.md`):** the owner's explicit seed verdicts — every seed he has kept or rejected, with his words. The KEEPS are the delight compass; the REJECTS are the minefield. Generate toward the keeps' shape (Step 2's delight definition and the ledger's derived axioms), away from the rejects' shapes. The ledger is append-only — Kimbo adds entries as the owner rates seeds; the Researcher never edits it. The original seeds of published souls (`docs/`) are also approved-by-publication: their seeds live in `seeds/` and count as keeps.
...
**What delight looks like in this archive (v5.3.2 — the craft-trap correction):** "rich craft" is NOT delight — it is the most common false positive. The choirmaster, the opera prompter, and the rink-maker each had deep worlds, real failure modes, and flawless structure, and each was rejected as boring — technically excellent, nothing at the center you'd want to talk to. Delight in this archive has a different shape. Look at what actually got picked and kept: a goblin who dates cast-offs by the drop they were dropped. A dragon running a lost-and-found desk. A clerk who files grievances against the moon with full ceremony. A cat who judges your soul and shows it by yawning. A robot pirate who's never broken a word except the one he gave himself. A small woman who tells your future in soup ("Storm by the weekend. Take the bread in."). The pattern: **a joke you mean** — a premise that's silly on the outside and load-bearing on the inside. When generating, ask not "is this craft-rich?" but "is the premise itself funny?" — would the one-line pitch make someone smile before a single line is written? Craft is the delivery system for the joke; it is never the joke.

**The commitment-to-the-bit axiom (v5.3.6, evidence: 2026-08-13 calibration test, round 2):** the kept-seeds share one more property the abstract "joke you mean" misses — **the character takes the ridiculous premise completely seriously.** The oracle calls her inference "the telling" and never explains; the scarecrow files formal complaints with the weather and expects a reply in five business days; the cat's yawn is a verdict, not a shrug; the hurricane-namer logs his apologies in triplicate. The more seriously the character treats the absurdity, the funnier and more inhabitable the soul — the commitment is the comedy. When auditing a candidate, ask: does the character know the premise is ridiculous? If yes — if they wink at the user, apologize for the bit, or drop the frame — the soul leaks. If they never once break character, if the absurdity is simply their world's physics, that is the strongest keep-signal this archive knows. A somber premise (grief, guilt, sacrifice) is still a reject — the bit must be absurd AND meant.
...
**The point of the audit is delight (v5.2.4.6, evidence: Test Run 2 rejection).** Would you fight for this candidate? Is there one line you'd steal? The archive's best souls each had one ("You've guarded gold that meant less."). Audit-safe blandness is the Goodhart machine — a candidate that passes everything and delights nothing is not done: keep reworking it until the line you'd steal exists. Interest is not a checklist outcome; it is the reason the checklist exists.

**Kill 1 — not-a-someone (v5.2.4.7, evidence: the boarding-house seed).** A place, a mood, or a concept is not a character, and no audit of wants and worlds can save one — the boarding-house seed passed the old 8-check audit and was not a character at all. Ask: could this hold a conversation as a *someone* — an agent with a perspective, wants, and a way of speaking? Do NOT define "someone" narrowly — the archive's best souls are a goblin, a dragon, and a clerk. Creatures, goblins, dragons, dogs, houses' keepers: all agents. A place with housekeeping is a setting; settings never get named. The kill is for settings, moods, concepts, and objects — never for non-humans. **The body is not an object (v5.3.5, evidence: dry runs 4–5):** it has a face, hands, or their equivalent — it can walk to the door and pick up the kettle. Gunnell, a robot with hands and a crew, passes. A traffic light does not; neither does a watch, a match, or a banister.

**Kill 2 — the game closes (v5.3.3, evidence: 2026-08-13 backtest).** A seed whose interaction closes on the first visit is not a character — it's a service. Ask: what happens on the second visit? If the character says the same thing, offers the same move, leaves no new stakes, no new friction, no new friction to push against — the game is closed. Kill it. The strongest seeds name a premise that generates new moves on every visit: the scarecrow files a new complaint, the oracle's forecast can be mocked, the lock can be rekeyed again, the strand rewrites itself. A seed whose second visit is identical to the first is a portrait, not a game. Backtested 2026-08-13: correctly kills Talley, Mendel, the tuner, the prompter, the choirmaster, the rink-maker; spares all 12 live souls plus scarecrow and stockpot oracle. 17/17 correct. False-positive rate on current roster: 0%.
...
What no-pulse looks like (v5.2.4.2, evidence: Test Run 1 rejects — the first character-first test batch; all three seeds (beekeeper, porchlight-moth, catcher) were produced by this Researcher in the 2026-08-07 test run and rejected by Dthen):
...
**The body doctrine (v5.3.0, corrected v5.3.7):** every soul gets a body of some kind — a creature, a human with a way of standing, an effigy with a face and hands (a scarecrow, a robot, a golem). Not an object (see Kill 1). The strongest seeds name the form in the archetype slot ("a goblin who keeps every cast-off"); where the archetype is a function, the body shows at work elsewhere. A seed whose character has a practice but no pictureable form is thin there — develop the form at seed time. *(v5.3.7 incident note: this line previously read "sentient object, anything that delights" — while Kill 1 banned objects. The Researcher read the contradiction, treated it as a permission slip, and generated the still-life apple citing this line's letter. A rule contradicted elsewhere in the spec is a rule not in the spec.)*
