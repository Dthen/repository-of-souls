...
**Idea-time checklist (apply BEFORE committing to any candidate — v5.2.4.3, research-synthesized).** The candidate must be BORN with all of the following — not filtered for them after the seed is written. Kill before writing if any check fails:
1. **A want, not a mood.** An observable goal it pursues and would sacrifice for. "Wants to help people" is a job description (CI: research-character-interest.md:323). Wanting is the engine — characters who yearn make fascinating choices (CI:46). A fantasy that is only a sentiment is dead.
2. **A want/need gap plus a lie believed.** The want conflicts with the need; a false belief blocks the need (CI:105–114). No gap = no engine (research-character-creation.md:18–20).
3. **A contradiction describable with "but"** — and it must be generative: two truths that produce choices, not a definition (CI:67; research-inhabitation-vs-description.md:287–294). Prefer social/paradox tension (research-success-patterns-v5.md:39–52).
4. **A world with nouns and verbs** — tools, materials, rhythms, failure modes. "Does this archetype have specific tools, materials, rhythms, and failure modes? If the answer is no, the seed fails. Reject abstract roles" (research-failure-modes.md:180; success-patterns-v5.md:417 — the gleaner's field and basket beat Roche's abstract philosophy). Job not required; world required. A creature or genre-cross qualifies only if it has its own material practice — a porch light has none.
5. **An interior that produces behavior** — answer at birth: what does it notice first? what does it avoid noticing? what would it never say out loud? what's the say/do contradiction? (research-internal-life.md:313–319). One gesture is not an interior; a gripe that stays passive is not vitality (IL:264).
6. **A diagnostic eye** — a way of seeing no one else has (success-patterns-v5.md:107–123) — plus an emotional gear with a counter-register: "a register without a counter-register is flat" (research-emotional-register.md:323,349).
7. **A first-impression anchor** — one concrete, sensorily-specific detail (research-character-memorability.md:227,21; character-creation.md:104–110). Abstract = forgotten.
8. **50-turn improv capacity** — can you improvise three DIFFERENT conversations with this character right now? If the answer requires inventing new character on the fly, kill (proposal v5.2:82).
