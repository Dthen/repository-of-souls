...
**Experiments:** **E1 (running free):** the E2E trio — Munday, Verrall, Rundle — are premise-comedic seeds through the UNCHANGED forge. Prediction: they survive. Voice-died, premise-live = H holds, fix is narrow. **⚠ CONFOUND LOGGED (self-caught, same day):** the reference-accuracy sweep (archive rosters, SOUL_VERDICTS pointer, "do not copy archived register") landed in stage-writer.md and stage-evaluator.md while the trio's Writers were reading specs at runtime — E1 is no longer a clean "unchanged forge" test. Trio results now evidence "premise-comedy under new references," not "premise-comedy under old forge." **E2 (hand-regen Cobbold A/B)** is therefore the primary controlled test of the compression hypothesis — same seed, same name, same budget, one rule added. **E3:** at ≥6 soul verdicts, run the proven playbook: blind judge predicts the owner's soul verdicts from seed+draft pairs against SOUL_VERDICTS.md; whatever discriminates becomes the Evaluator's fix.
...
**Annotation (appended same day, 2026-09-13, by the Featherstone Evaluator — not a spec edit, E2/E3 input):** the first voice-comic draft through the NEW references was Featherstone (test6 seed, the Dolores monologue), and it compressed exactly as H predicts — confession fragment kept in the name file, zero self-utterance lines in the draft, monologue surviving only as allusion. It was REJECTED at the gate (evaluations/featherstone.md) where the same shape passed for Suttle/Cobbold/Underhill, so gate lethality on voice-comedy is now an open variable, not a constant: the evidence for a first voice-comic **REJECT at the gate** sits here; E2/E3 should score whether the surviving duds' failure was the gate or the format.
